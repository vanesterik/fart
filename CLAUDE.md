...
`fart train` takes the same `data_dir`/`market`/`interval` positional arguments plus a `--months` option (default `6`, most recent months of candle history to train on) — e.g. `uv run fart train assets BTC-EUR 1h --months 3`. Loads the cached CSV for that market/interval, computes technical indicators, time-series-splits it, then fits the quick-prototype `NBeatsNet` (hyperparameters in `fart/model/nbeats_config.py::NBeatsConfig`) and logs per-candle magnitude/confidence — results aren't yet persisted or connected to a signal/predict path.
