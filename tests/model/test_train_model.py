...
def test_train_model_reduces_loss() -> None:
    ...
    trained, loss_history = train_model(
        model=model,
        x_train=x,
        y_train=y,
        batch_size=16,
...
def test_train_model_with_validation_returns_per_epoch_loss_history() -> None:
    rng = np.random.default_rng(0)
    ...
    assert [record["epoch"] for record in loss_history] == [1.0, 2.0, 3.0]
